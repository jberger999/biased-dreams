...
class Actor(nn.Module):
    ...
    def get_greedy_action_and_log_prob(self, in_features: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        mean, std = self._get_base_dist(in_features=in_features)
        # Use the mean for the action.
        untransformed_sample = mean
    ...
    def get_log_prob(self, in_features: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        mean, std = self._get_base_dist(in_features=in_features)
        untransformed_sample = action
        transformed_sample_log_prob = self._tanh_log_prob(untransformed_sample, mean, std)
        return transformed_sample_log_prob
