...
class ReplayBuffer:
    ...
    def _collect_values(self,
        ...
        all_obs, all_actions, all_rewards, all_phys_states = [], [], [], []
        ...
        for k, v in self._all_data.items():
            if use_keys is None or k in use_keys:
                ...
                all_phys_states += [v["infos"][i]["state"] for i in range(len(v["infos"]))]
                if self._has_loss_masks:
                    all_loss_masks += [d["loss_mask"] for d in v["infos"]]
        return all_obs, all_actions, all_rewards, all_phys_states, all_loss_masks
    ...
    def collect_states(self) -> torch.Tensor:
        all_states = []
        for k, v in self._all_data.items():
            if "state" in v["infos"][0].keys():
                all_states += [d["state"] for d in v["infos"]]
        all_states = torch.stack(all_states, dim=0)
        dims = all_states.shape[-1]
        all_states = all_states.reshape(-1, dims)
        return all_states
    
    def collect_states_actions(self) -> dict:
        dataset = {}
        for k, v in self._all_data.items():
            dataset[k] = {"states": [d["state"] for d in v["infos"] if "state" in d.keys()],
                          "actions": v["actions"]}
        return dataset
    ...
    def get_data_loader(self,
                        ...
                        seq_length: int,
                        use_keys: Optional[list[str]] = None):
        obs, acts, rewards, phys_states, loss_masks = self._sample_seqs(batch_size=batch_size,
    ...
    def _sample_seqs(self,
        ...
        all_obs, all_actions, all_rewards, all_phys_states, all_loss_masks = self._collect_values(use_keys=use_keys)

        num_samples = batch_size * num_batches
        seq_idx = np.random.randint(low=0, high=len(all_actions), size=num_samples)
        obs, acts, rewards, phys_states = [], [], [], []
    ...
    def _build_dataloader(self,
        ...
        idx_dict["phys_states"] = len(data_tensors)
        data_tensors.append(phys_states)
    ...
    def _collate_fn(self,
        ...
        phys_states = data_batch[idx_dict["phys_states"]].to(device, non_blocking=True)

        if self._has_loss_masks:
            loss_masks = []
            for i in range((num_obs + 1) + 1): # + 1, since phys state decoder has no corresponding obs
        ...
        return obs_batch, obs_batch + [rewards, phys_states], prev_action_batch, loss_masks
