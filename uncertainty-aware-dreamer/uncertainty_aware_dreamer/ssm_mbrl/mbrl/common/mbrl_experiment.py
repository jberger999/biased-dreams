...
class MBRLExperiment:
    ...
    def build(self,
        ...
        if self._verbose:
            print("=== MBRL ===")
            print(OmegaConf.to_yaml(mbrl_config))
        ...
        output_sizes = obs_sizes + [1] # (img, reward)
        if model_config.use_phys_state_dec:
            output_sizes += [base_env.get_phys_state_size()] # (img, reward, phys_state)
        self._model = self._model_factory.build(config=model_config,
                                                input_sizes=obs_sizes, # (img)
                                                output_sizes=output_sizes,
                                                action_dim=base_env.action_dim).to(self._device)

        # Ensemble, if applicable
        if self._ensemble_factory is not None:
            self._conf_save_dict["ensemble_config"] = ensemble_config
            if self._verbose:
                print("=== Ensemble ===")
                print(OmegaConf.to_yaml(ensemble_config))
            
            self._ensemble = self._ensemble_factory.build(world_model=self._model,
                                                          config=ensemble_config).to(self._device)

        # Policy
        self._conf_save_dict["policy_config"] = policy_config
        if self._verbose:
            print("=== Policy ===")
            print(OmegaConf.to_yaml(policy_config))
    ...
    def get_states(self):
        return self._replay_buffer.collect_states()
    
    def get_states_actions(self):
        return self._replay_buffer.collect_states_actions()
    ...
    def save_models(self, path: str):
        if self._ensemble is not None:
            torch.save({"model": self._model.state_dict(),
                        "actor": self._policy.actor.state_dict(),
                        "critic": self._policy.critic.state_dict(),
                        "ensemble": self._ensemble.state_dict()}, 
                        path + "/networks.pth")
        else:
            torch.save({"model": self._model.state_dict(),
                        "actor": self._policy.actor.state_dict(),
    ...
    def load_models(self, path: str):
        networks = torch.load(path + "/networks.pth")
        self._model.load_state_dict(networks["model"])
        self._policy.actor.load_state_dict(networks["actor"])
        self._policy.critic.load_state_dict(networks["critic"])
        if self._ensemble is not None:
            self._ensemble.load_state_dict(networks["ensemble"]) 