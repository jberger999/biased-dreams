...
class GaussianMLP(Ensemble):
    ...
    def __init__(
        ...
        def create_activation():
            ...
            # Case 1: already instantiated module
            if isinstance(activation_fn_cfg, nn.Module):
                return activation_fn_cfg

            # Case 2: class (e.g. torch.nn.SiLU)
            if isinstance(activation_fn_cfg, type):
                return activation_fn_cfg()

            # Case 3: dict / DictConfig
            if isinstance(activation_fn_cfg, (dict, omegaconf.DictConfig)):
                return hydra.utils.instantiate(activation_fn_cfg)

            raise ValueError(f"Unsupported activation config type: {type(activation_fn_cfg)}")
    ...
    def load(self, load_dir: Union[str, pathlib.Path]):
        ...
        model_dict = torch.load(pathlib.Path(load_dir) / self._MODEL_FNAME, weights_only=False) # weights_only=False, since load for hopper then doesnt work for some reason
